...
def confirm_payment():
    if not cart:
        messagebox.showwarning("ตะกร้าว่าง", "ไม่มีสินค้าในตะกร้า")
        return

    total_amount = sum(
        quantity * c.execute("SELECT price FROM knives WHERE name = ?", (name,)).fetchone()[0]
        for name, quantity in cart.items()
    )
    
    # ให้ผู้ใช้กรอกชื่อผู้ใช้
    username = simpledialog.askstring("ชื่อผู้ใช้", "กรุณาใส่ชื่อผู้ใช้:")
    if not username:
        messagebox.showwarning("ชื่อผู้ใช้ไม่ถูกต้อง", "กรุณาใส่ชื่อผู้ใช้ให้ถูกต้อง")
        return

    # บันทึกประวัติการทำรายการ
    save_transaction_history(username, total_amount)

    # เคลียร์ตะกร้าหลังจากชำระเงินสำเร็จ
    cart.clear()
    refresh_cart_list(cart_tree)

    messagebox.showinfo("ชำระเงินสำเร็จ", f"ขอบคุณ {username} สำหรับการซื้อสินค้า ราคารวม {total_amount} บาท")    

def show_transaction_history():
    history_window = Toplevel(root)
    history_window.title("ประวัติการทำรายการ")

    tree = ttk.Treeview(history_window, columns=("ID", "Username", "Amount", "Time"), show="headings")
    tree.heading("ID", text="ID")
    tree.heading("Username", text="ชื่อผู้ใช้")
    tree.heading("Amount", text="จำนวนเงิน")
    tree.heading("Time", text="เวลาทำรายการ")
    ...
    try:
        ...
        rows = c.fetchall()
        if not rows:
            messagebox.showinfo("ไม่มีประวัติ", "ยังไม่มีการทำรายการ")
        else:
            for row in rows:
                tree.insert("", "end", values=row)
    except Exception as e:
        messagebox.showerror("ข้อผิดพลาด", f"ไม่สามารถดึงข้อมูลได้: {e}")
...
history_button = ttk.Button(button_frame, text="ประวัติการทำรายการ", command=show_transaction_history)
...
purchase_button = ttk.Button(sell_product_frame, text="แอดลงตะกร้า", command=purchase_product)
