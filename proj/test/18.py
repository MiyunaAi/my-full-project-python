...
def add_to_cart():
    ...
    if selected_item:
        ...
        if quantity > 0:
            # ตรวจสอบจำนวนสินค้าว่าพอหรือไม่
            ...
            if result:
                ...
                if stock >= quantity:
                    ...
                    messagebox.showinfo("Success", f"เพิ่ม {name} จำนวน {quantity} ชิ้นลงในตะกร้าเรียบร้อยแล้ว!")
                    refresh_cart_list(cart_tree)
                else:
                    messagebox.showwarning("Stock Warning", f"สินค้ามีไม่เพียงพอ (มีเพียง {stock} ชิ้นในสต็อก)")
            else:
                messagebox.showerror("Error", "ไม่พบสินค้าที่เลือก")
        else:
            messagebox.showwarning("Invalid Input", "กรุณาใส่จำนวนที่ถูกต้อง")
    else:
        messagebox.showwarning("Warning", "กรุณาเลือกสินค้าที่ต้องการซื้อ")

# ฟังก์ชันรีเฟรชรายการตะกร้า
def refresh_cart_list(treeview):
    treeview.delete(*treeview.get_children())
    for item in cart:
        total_price_per_item = item['price'] * item['quantity']
        treeview.insert("", "end", values=(item['name'], item['quantity'], item['price'], total_price_per_item))
    ...
    if current_user:
        if cart:
            ...
            for item in cart:
                ...
                # ลดสต็อกสินค้าหลังสั่งซื้อ
                c.execute("UPDATE knives SET stock = stock - ? WHERE name = ?", (quantity, name))
                # เพิ่มข้อมูลประวัติการสั่งซื้อ
                c.execute("INSERT INTO purchase_history (username, knife_name, quantity, total_price) VALUES (?, ?, ?, ?)",
                          (current_user, name, quantity, item['price'] * quantity))
            conn.commit()
            messagebox.showinfo("Order Success", "สั่งซื้อสินค้าสำเร็จ!")
            cart.clear()
            refresh_cart_list(cart_tree)
            refresh_knife_list(buy_knife_tree)
        else:
            messagebox.showwarning("Cart Warning", "ตะกร้าสินค้าว่างเปล่า")
    else:
        messagebox.showwarning("Login Error", "กรุณาล็อกอินก่อนทำการสั่งซื้อ")
...
buy_knife_tree = ttk.Treeview(buy_frame, columns=("Name", "Stock", "Price"), show="headings")
...
# ปรับให้เนื้อหาทั้งหมดในตารางอยู่ตรงกลาง
buy_knife_tree.column("Name", anchor='center')
buy_knife_tree.column("Stock", anchor='center')
buy_knife_tree.column("Price", anchor='center')

# ช่องใส่จำนวนสินค้าที่ต้องการซื้อ
quantity_label = ttk.Label(buy_frame, text="จำนวน:")
quantity_label.pack(pady=5)
quantity_entry = ttk.Entry(buy_frame)
quantity_entry.pack(pady=5)

add_to_cart_button = ttk.Button(buy_frame, text="เพิ่มลงตะกร้า", command=add_to_cart)
add_to_cart_button.pack(pady=5)

# สร้าง Frame สำหรับการแสดงตะกร้า
cart_tree = ttk.Treeview(cart_frame, columns=("Name", "Quantity", "Price", "Total"), show="headings")
cart_tree.heading("Name", text="ชื่อมีด")
cart_tree.heading("Quantity", text="จำนวน")
cart_tree.heading("Price", text="ราคา")
...
confirm_order_button = ttk.Button(cart_frame, text="ยืนยันการสั่งซื้อ", command=confirm_order)
confirm_order_button.pack(pady=5)

# ตารางแสดงรายการมีดสำหรับการซื้อ
buy_tree = ttk.Treeview(buy_frame, columns=("Name", "Stock", "Price", "Total Price"), show="headings")
buy_tree.heading("Name", text="ชื่อมีด")
buy_tree.heading("Stock", text="จำนวนในสต็อก")
buy_tree.heading("Price", text="ราคา")
buy_tree.heading("Total Price", text="ราคารวม")
buy_tree.pack(pady=10)

# ปุ่มสำหรับซื้อมีด
buy_button = ttk.Button(buy_frame, text="เลือกซื้อมีด", command=buy_knive)
buy_button.pack(pady=10)

# ตารางแสดงสินค้าที่อยู่ในตะกร้า
cart_tree = ttk.Treeview(cart_frame, columns=("Name", "Price", "Quantity", "Total Price"), show="headings")
cart_tree.heading("Name", text="ชื่อมีด")
cart_tree.heading("Price", text="ราคา")
cart_tree.heading("Quantity", text="จำนวน")
cart_tree.heading("Total Price", text="ราคารวม")
cart_tree.pack(pady=10)
