...
c = conn.cursor()
cart = {}  # เก็บชื่อสินค้าเป็นคีย์ และจำนวนสินค้าเป็นค่า
...
def refresh_cart_list(treeview):
    ...
    cart_total_label.config(text=f"ราคารวมทั้งหมด: {total} บาท")
...
def add_to_cart():
    ...
    if selected_item:
        ...
        if name in cart:
            ...
            cart[name] = 1
        messagebox.showinfo("Success", f"เพิ่ม {name} ลงในตะกร้าแล้ว!")

def load_products():
    sell_product_tree.delete(*sell_product_tree.get_children())  # ลบข้อมูลเก่า

    # ดึงข้อมูลสินค้าจากฐานข้อมูล (หรือใช้ข้อมูลตัวอย่าง)
    c.execute("SELECT name, price FROM knives")
    products = c.fetchall()

    # เพิ่มสินค้าใน Treeview
    for product in products:
        sell_product_tree.insert("", "end", values=product)

def show_sell_product_window():
    for frame in content_frames:
        frame.pack_forget()  # ซ่อนหน้าต่างอื่น
    sell_product_frame.pack(fill="both", expand=True)
    load_products()  # โหลดข้อมูลสินค้า
...
content_frames = [login_frame, register_frame, home_frame, add_knife_frame, remove_knife_frame, knife_list_frame, checkout_frame,sell_product_frame,cart_frame]
...
sell_button = ttk.Button(button_frame, text="ขายสินค้า", command=sell_product_window)
...
# ปุ่มกลับไปยังหน้าหลัก (กรณีต้องการ)
back_button = ttk.Button(sell_product_frame, text="กลับไปหน้าหลัก", command=show_home_window)
back_button.pack(pady=5)


cart_tree = ttk.Treeview(cart_frame, columns=("Name", "Quantity", "Price", "Total Price"), show="headings")
...
cart_total_label = ttk.Label(cart_frame, text="ราคารวมทั้งหมด: 0 บาท")
...
checkout_button = ttk.Button(cart_frame, text="ชำระเงิน", command=confirm_payment)
