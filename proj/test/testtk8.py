...
#root.tk.call("source", "azure-dark.tcl")  # หากต้องการใช้ธีมที่ดูทันสมัย
#style.theme_use("azure-dark")

# สร้างส่วนต่าง ๆ เป็น Frame
# ส่วนเพิ่มมีด
add_frame = tk.Frame(root, bg="#2a2a2a")
ttk.Label(add_frame, text="ชื่อมีด", font=('Arial', 12)).pack(pady=5)
knife_name_entry = ttk.Entry(add_frame, font=('Arial', 12))
knife_name_entry.pack(pady=5)
ttk.Label(add_frame, text="ราคา", font=('Arial', 12)).pack(pady=5)
knife_price_entry = ttk.Entry(add_frame, font=('Arial', 12))
knife_price_entry.pack(pady=5)
ttk.Label(add_frame, text="จำนวน", font=('Arial', 12)).pack(pady=5)
knife_stock_entry = ttk.Entry(add_frame, font=('Arial', 12))
knife_stock_entry.pack(pady=5)
add_button = ttk.Button(add_frame, text="เพิ่มมีด", command=add_knife)
add_button.pack(pady=10)

# ส่วนลดจำนวนสินค้า
reduce_frame = tk.Frame(root, bg="#2a2a2a")
ttk.Label(reduce_frame, text="ลดจำนวนสินค้า", font=('Arial', 12)).pack(pady=5)
stock_reduce_entry = ttk.Entry(reduce_frame, font=('Arial', 12))
...
# ส่วนลบมีด
remove_frame = tk.Frame(root, bg="#2a2a2a")
remove_button = ttk.Button(remove_frame, text="ลบมีด", command=remove_knife)
remove_button.pack(pady=10)

# ส่วนแสดงมีดทั้งหมด
show_frame = tk.Frame(root, bg="#2a2a2a")
show_button = ttk.Button(show_frame, text="แสดงมีดทั้งหมด", command=show_knives)
show_button.pack(pady=10)

# ส่วนการซื้อมีด
purchase_frame = tk.Frame(root, bg="#2a2a2a")
ttk.Label(purchase_frame, text="จำนวนที่ต้องการซื้อ", font=('Arial', 12)).pack(pady=5)
purchase_quantity_entry = ttk.Entry(purchase_frame, font=('Arial', 12))
...
toggle_add_button.text = "เพิ่มมีด"
toggle_add_button.pack(pady=10)
...
toggle_purchase_button.text = "ซื้อมีด"
toggle_purchase_button.pack(pady=10)
