...
def sell_product_window():
    ...
    image_label.pack(padx=10,pady=20)  # เพิ่มระยะห่างด้านบน
...
def confirm_order():
    ...
    promptpay_data = f"00020101021129370010{total_price}0000000000000000" \
                     f"01160004{total_price}00000000000" \
                     f"05115427011{958620453}020{total_price}0300" \
                     f"0000000030000010000000000" \
                     f"000000000000000000000000000000"
    
    qr_code = qrcode.make(promptpay_data)
    
    # Save the QR code as an image
    qr_code.save("promptpay_qrcode.png")

    # Create a new window to display the QR code
    qr_window = tk.Toplevel(root)
    qr_window.title("QR Code for Payment")

    qr_img = Image.open("promptpay_qrcode.png")
